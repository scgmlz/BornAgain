...
bool calculateUpFromLayer(
    SpecularMatrix::MultiLayerCoeff_t& coeff, const MultiLayer& sample, const kvector_t k,
    size_t layer_index);
    ...
    size_t layer_index)
{
    for (int i = layer_index; i >= 0; --i) {
        ...
        coeff[i].t_r(0) = ((lambda_rough + lambda_below) * coeff[i + 1].t_r(0)
                           + (lambda_rough - lambda_below) * coeff[i + 1].t_r(1))
            / 2.0 / lambda * std::exp(-ikd * lambda);
        coeff[i].t_r(1) = ((lambda_rough - lambda_below) * coeff[i + 1].t_r(0)
                           + (lambda_rough + lambda_below) * coeff[i + 1].t_r(1))
            / 2.0 / lambda * std::exp(ikd * lambda);
