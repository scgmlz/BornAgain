...
std::vector<MatrixRTCoefficients_v2> SpecularMagneticStrategy::eval(const std::vector<Slice>& slices,
                                                              const kvector_t& k) const
{
    return eval(slices, KzComputation::computeReducedKz(slices, k));
}
...
void SpecularMagneticStrategy::nullifyBottomReflection(MatrixRTCoefficients_v2& coeff) const
{
    const complex_t l_1 = coeff.m_lambda(0);
    const complex_t l_2 = coeff.m_lambda(1);
...
Eigen::Matrix2cd
SpecularMagneticStrategy::findNormalizationCoefficients(const MatrixRTCoefficients_v2& coeff) const
{
