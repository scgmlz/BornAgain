    ...
    const MultiLayer& sample, const kvector_t k, MultiLayerCoeff_t& coeff)
{
    ...
    for (size_t i = 0; i < coeff.size(); ++i) {
        coeff[i].m_scatt_matrix = sample.getLayer(i)->getMaterial()->getSpecularScatteringMatrix(k);
        coeff[i].m_kt = mag_k * sample.getLayer(i)->getThickness();
        coeff[i].m_a = coeff[i].m_scatt_matrix.trace() / 2.0;
        coeff[i].m_b_mag =
            sqrt(coeff[i].m_a * coeff[i].m_a - (complex_t)coeff[i].m_scatt_matrix.determinant());
    ...
    const MultiLayer& sample, const kvector_t k, MultiLayerCoeff_t& coeff)
{
    ...
    for (int i = (int)N - 2; i > 0; --i) {
        ...
        Eigen::Matrix4cd l = coeff[i].R1m * getImExponential((complex_t)(coeff[i].kz(0) * t))
