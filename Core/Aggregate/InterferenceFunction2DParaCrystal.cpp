...
InterferenceFunction2DParaCrystal::InterferenceFunction2DParaCrystal(
    ...
    : m_integrate_xi(false), m_damping_length(damping_length), m_use_damping_length(true)
{
...
{
    InterferenceFunction2DParaCrystal* p_new = new InterferenceFunction2DParaCrystal(
        peak_distance, peak_distance, M_PI_2, 0.0, damping_length);
...
complex_t
InterferenceFunction2DParaCrystal::FTPDF(double qx, double qy, double xi, size_t index) const
{
    double length = (index ? m_lattice_params.m_length_2 : m_lattice_params.m_length_1);
