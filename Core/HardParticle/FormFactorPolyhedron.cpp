...
complex_t PolyhedralEdge::contrib(int M, const cvector_t qpa, complex_t qrperp) const
{
    ...
    if (v1 == 0.) {
        ...
        for (int mm = 1; mm <= M; ++mm) {
            complex_t term = precomputed.reciprocal_factorial[mm]
                * precomputed.reciprocal_factorial[M - mm] * pow(v2, mm) * pow(v1, M - mm);
            ret += term;
#ifdef POLYHEDRAL_DIAGNOSTIC
    ...
    for (int l = 1; l <= M / 2; ++l) {
        complex_t term = precomputed.reciprocal_factorial[M - 2 * l]
            * precomputed.reciprocal_factorial[2 * l + 1] * pow(u, 2 * l) * pow(v, M - 2 * l);
        ret += term;
#ifdef POLYHEDRAL_DIAGNOSTIC
        if (diagnosis.debmsg >= 6)
    ...
    const PolyhedralTopology& topology, double z_origin, const std::vector<kvector_t>& vertices)
{
    try {
        ...
        throw std::logic_error(
            "Bug in " + getName() + ": " + e.what() + " [please report to the maintainers]");
    } catch (std::exception& e) {
        throw std::runtime_error(
            "Unexpected exception in " + getName() + ": " + e.what()
