...
HomogeneousMagneticMaterial::HomogeneousMagneticMaterial(
    ...
    : HomogeneousMaterial(name, refractive_index), m_magnetic_field(magnetic_field)
{
...
Eigen::Matrix2cd HomogeneousMagneticMaterial::getScatteringMatrix(double k_mag2) const
{
    Eigen::Matrix2cd result;
    double factor = m_magnetic_prefactor / k_mag2;
    complex_t unit_factor = m_refractive_index * m_refractive_index;
    result = unit_factor * m_unit_matrix + factor * m_pauli_operator[0] * m_magnetic_field[0]
...
const IMaterial*
HomogeneousMagneticMaterial::createTransformedMaterial(const Transform3D& transform) const
{
    kvector_t mag_field_transformed = transform.transformed(m_magnetic_field);
    return new HomogeneousMagneticMaterial(getName(), getRefractiveIndex(), mag_field_transformed);
}
