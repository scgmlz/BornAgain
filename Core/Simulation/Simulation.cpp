...
Simulation::Simulation(const MultiLayer& p_sample) { mP_sample.reset(p_sample.clone()); }
...
void Simulation::setBeamIntensity(double intensity) { m_instrument.setBeamIntensity(intensity); }

double Simulation::getBeamIntensity() const { return m_instrument.getBeamIntensity(); }
...
void Simulation::prepareSimulation() { gsl_set_error_handler_off(); }
...
void Simulation::runSimulation()
{
    ...
    m_progress.reset();
    int prefac = (mP_sample->totalNofLayouts() > 0 ? 1 : 0) + (mP_sample->hasRoughness() ? 1 : 0);
    ...
    for (size_t index = 0; index < param_combinations; ++index) {
        ...
        runSingleSimulation();
        addElementsWithWeight(
            m_sim_elements.begin(), m_sim_elements.end(), total_intensity.begin(), weight);
    }
...
void Simulation::setSample(const MultiLayer& sample) { mP_sample.reset(sample.clone()); }
...
void Simulation::addParameterDistribution(
    ...
    if (m_options.getNumberOfThreads() == 1) {
        ...
        if (!P_dwba_simulation->isCompleted()) {
            ...
            throw Exceptions::RuntimeErrorException(
                "Simulation::runSimulation() -> Simulation has "
        ...
        for (int i_thread = 0; i_thread < m_options.getNumberOfThreads(); ++i_thread) {
            ...
            std::vector<SimulationElement>::iterator begin_it =
                batch_start + i_thread * element_thread_step;
            ...
            simulations.push_back(
                new MainComputation(mP_sample.get(), m_options, m_progress, begin_it, end_it));
        }
