...
void RectangularDetector::setPosition(
    const kvector_t normal_to_detector, double u0, double v0, const kvector_t direction)
{
    m_detector_arrangement = GENERIC;
...
IPixelMap* RectangularDetector::createPixelMap(size_t index) const
{
    ...
    kvector_t corner_position(
        m_normal_to_detector + (u_bin.m_lower - m_u0) * m_u_unit
...
size_t RectangularDetector::getNbinsX() const { return getAxis(BornAgain::X_AXIS_INDEX).size(); }

size_t RectangularDetector::getNbinsY() const { return getAxis(BornAgain::Y_AXIS_INDEX).size(); }

kvector_t RectangularDetector::getNormalVector() const { return m_normal_to_detector; }

double RectangularDetector::getU0() const { return m_u0; }

double RectangularDetector::getV0() const { return m_v0; }

kvector_t RectangularDetector::getDirectionVector() const { return m_direction; }

double RectangularDetector::getDistance() const { return m_distance; }

double RectangularDetector::getDirectBeamU0() const { return m_dbeam_u0; }

double RectangularDetector::getDirectBeamV0() const { return m_dbeam_v0; }
...
std::vector<IDetector2D::EAxesUnits> RectangularDetector::getValidAxesUnits() const
{
    ...
    std::vector<IDetector2D::EAxesUnits> addon = { IDetector2D::RADIANS, IDetector2D::DEGREES,
                                                   IDetector2D::MM, IDetector2D::QYQZ };
    result.insert(result.end(), addon.begin(), addon.end());
    return result;
}

IDetector2D::EAxesUnits RectangularDetector::getDefaultAxesUnits() const { return IDetector2D::MM; }
...
void RectangularDetector::calculateAxisRange(
