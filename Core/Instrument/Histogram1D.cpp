...
Histogram1D::Histogram1D(const OutputData<double>& data) { init_from_data(data); }

Histogram1D* Histogram1D::clone() const { return new Histogram1D(*this); }
...
std::vector<double> Histogram1D::getBinCenters() const { return getXaxis().getBinCenters(); }
