...
IDetector2D::IDetector2D() : m_axes() { init_parameters(); }
...
void IDetector2D::setDetectorParameters(
...
void IDetector2D::setAnalyzerProperties(
...
const RegionOfInterest* IDetector2D::regionOfInterest() const { return m_region_of_interest.get(); }
...
void IDetector2D::removeMasks() { m_detector_mask.removeMasks(); }
...
const DetectorMask* IDetector2D::getDetectorMask() const { return &m_detector_mask; }
...
bool IDetector2D::isMasked(size_t index) const { return m_detector_mask.isMasked(index); }

bool IDetector2D::hasMasks() const { return m_detector_mask.hasMasks(); }
...
std::vector<SimulationElement> IDetector2D::createSimulationElements(const Beam& beam)
{
    ...
    for (SimulationArea::iterator it = area.begin(); it != area.end(); ++it) {
        SimulationElement sim_element(
            wavelength, alpha_i, phi_i,
            std::unique_ptr<IPixelMap>(createPixelMap(it.detectorIndex())));
...
SimulationElement IDetector2D::getSimulationElement(size_t index, const Beam& beam) const
{
    ...
    return SimulationElement(
        wavelength, alpha_i, phi_i, std::unique_ptr<IPixelMap>(createPixelMap(index)));
}
...
IDetector2D::constructAxis(size_t axis_index, const Beam& beam, IDetector2D::EAxesUnits units) const
{
    double amin(0), amax(0);
    calculateAxisRange(axis_index, beam, units, amin, amax);

    std::unique_ptr<IAxis> result(
        new FixedBinAxis(getAxisName(axis_index), getAxis(axis_index).size(), amin, amax));
...
void IDetector2D::calculateAxisRange(
    size_t axis_index, const Beam& beam, IDetector2D::EAxesUnits units, double& amin,
    double& amax) const
{
    amin = 0.0;
    amax = 0.0;
    ...
    if (units == DEFAULT) {
        ...
        SimulationElement el_left_bottom = getSimulationElement(getGlobalIndex(0, 0), beam);
        ...
        throw Exceptions::RuntimeErrorException(
            "IDetector2D::calculateAxisRange() -> Error. "
            "Unknown units "
            + std::to_string(units));
    }
}
...
void IDetector2D::setDataToDetectorMap(
...
void IDetector2D::addAxis(const IAxis& axis) { m_axes.push_back(axis.clone()); }

size_t IDetector2D::getDimension() const { return m_axes.size(); }

void IDetector2D::clear() { m_axes.clear(); }
...
bool IDetector2D::isCorrectAxisIndex(size_t index) const { return index < getDimension(); }
