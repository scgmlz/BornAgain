...
MathFunctions::Convolve::Workspace::~Workspace() { clear(); }
...
void MathFunctions::Convolve::init(int h_src, int w_src, int h_kernel, int w_kernel)
{
    if (!h_src || !w_src || !h_kernel || !w_kernel) {
        ...
        os << "MathFunctions::Convolve::init() -> Panic! Wrong dimensions " << h_src << " " << w_src
    ...
    ws.p_forw_src = fftw_plan_dft_r2c_2d(
        ws.h_fftw, ws.w_fftw, ws.in_src, (fftw_complex*)ws.out_src, FFTW_ESTIMATE);
    if (ws.p_forw_src == nullptr)
        throw Exceptions::RuntimeErrorException(
            "MathFunctions::Convolve::init() -> Error! Can't initialise p_forw_src plan.");

    ws.p_forw_kernel = fftw_plan_dft_r2c_2d(
        ws.h_fftw, ws.w_fftw, ws.in_kernel, (fftw_complex*)ws.out_kernel, FFTW_ESTIMATE);
