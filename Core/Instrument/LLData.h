...
template <class T> LLData<T>::~LLData() { clear(); }
...
template <class T> inline T& LLData<T>::operator[](size_t i) { return m_data_array[i]; }

template <class T> inline const T& LLData<T>::operator[](size_t i) const { return m_data_array[i]; }
