...
template <> BasicVector3D<double> BasicVector3D<double>::conj() const { return *this; }
...
template <> double BasicVector3D<double>::cosTheta() const { return mag() == 0 ? 1 : z() / mag(); }
...
template <> BasicVector3D<double> BasicVector3D<double>::real() const { return *this; }
...
vecOfLambdaAlphaPhi(const double _lambda, const double _alpha, const double _phi)
{
    double k = PI2 / _lambda;
    return BasicVector3D<double>(
        k * std::cos(_alpha) * std::cos(_phi), -k * std::cos(_alpha) * std::sin(_phi),
        k * std::sin(_alpha));
}
