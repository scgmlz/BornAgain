...
TEST_F(SpecularMagneticTest, initial)
{
    ...
    std::make_unique<SpecularMagneticOldStrategy>()->eval(sample.slices(), v);
}
...
TEST_F(SpecularMagneticTest, zerofield)
{
    ...
    // k1
    coeffs_scalar = static_cast<SpecularScalarStrategy*>(
                sample_scalar.fresnelMap()->getStrategy())->eval(sample_scalar.slices(), k1);
    ...
    auto coeffs_zerofield = std::make_unique<SpecularMagneticOldStrategy>()->eval(sample_zerofield.slices(), k1);
    ...
    // k2
    coeffs_scalar = std::make_unique<SpecularScalarStrategy>()->eval(sample_scalar.slices(), k2);
    ...
    coeffs_zerofield = std::make_unique<SpecularMagneticOldStrategy>()->eval(sample_zerofield.slices(), k2);
    ...
    // k3
    coeffs_scalar = std::make_unique<SpecularScalarStrategy>()->eval(sample_scalar.slices(), k3);
    RTScalar = coeffs_scalar[1];
    TPS = RTScalar.T1plus() + RTScalar.T2plus();
    RPS = RTScalar.R1plus() + RTScalar.R2plus();
    TMS = RTScalar.T1min() + RTScalar.T2min();
    ...
    coeffs_zerofield = std::make_unique<SpecularMagneticOldStrategy>()->eval(sample_zerofield.slices(), k3);
    RTMatrix = coeffs_zerofield[1];
    TPM = RTMatrix.T1plus() + RTMatrix.T2plus();
    RPM = RTMatrix.R1plus() + RTMatrix.R2plus();
    TMM = RTMatrix.T1min() + RTMatrix.T2min();
