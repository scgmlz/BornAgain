...
{
    auto coeffs_scalar = std::make_unique<SpecularScalarStrategy>()->eval(sample_scalar.slices(), k);
    auto coeffs_zerofield = std::make_unique<SpecularMagneticStrategy>()->eval(sample_zerofield.slices(), k);
...
TEST_F(SpecularMagneticTest_v2, degenerate)
{
    ...
    auto result = std::make_unique<SpecularMagneticStrategy>()->eval(sample.slices(), v);
